...
class MLP(Classifier):
    ...
    def __init__(self, no_of_classes, no_of_inputs, no_hidden_neurons, activation, epochs=500, eta=0.01):
        self.precision = 10 ** (-7)
    ...
    def train(self, training_inputs, labels):
        ...
        for i in range(self.epochs):
            ...
            for inputs, label in zip(training_inputs, labels):
                ...
                H = np.c_[-1, H]
                Y = self.activate_output_neurons(H)
                ...
                output_multiplier = self.eta * output_gradient
                self.mm += np.dot(H.T, output_multiplier)
    ...
    def model_training(no_of_classes, no_of_attributes, X, y):
        ...
        y = np.array(y)
        hidden_neurons = {}
        for n in range(6, 10, 2):  # range from 4 to 12
            print("hidden neurons: {}".format(n))
            kf = KFold(n_splits=5)
            accuracies = []
            for train_index, test_index in kf.split(X):
                perceptron = MLP(no_of_classes, no_of_attributes, n, 'logistic')
    ...
    def plot_decision_boundaries_one(train_X, train_y, test_X, test_y, model, figure_index):
        ...
        plt.figure(figure_index)
        fig, ax = plt.subplots()
        plt.title("Artificial I - {} neurônios ocultos".format(10))
        plt.xlabel('Eixo X')
        plt.ylabel('Eixo y')
