...
def separateByClass(train_X, train_y):
    separated = {}
    for i in range(len(train_X)):
        vector = train_X[i]
        if train_y[-1] not in separated:
            separated[train_y[-1]] = []
        separated[train_y[-1]].append(vector)
    return separated
...
def summarizeByClass(train_X, train_y):
    separated = separateByClass(train_X, train_y)
    summaries = {}
