...
class Classifier(ABC):
    ...
    def plot_decision_boundaries(self, train_X, train_y, test_X, test_y, figure_index, window_h):
        ...
        self.fit(train_X, train_y, window_h)
    ...
    def model_training(self, x, y):
        ...
        y = np.array(y)
        h_values = {}
        values_range = [0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5]
        for h in values_range:
            kf = KFold(n_splits=5)
            accuracies = []
            for train_index, test_index in kf.split(x):
                ...
                self.fit(train_x, train_y, h)
                predictions = self.predict(test_x)
                accuracies.append(self.evaluate(test_y, predictions))

            h_values[h] = np.mean(accuracies)

        h_values = sorted(h_values.items(), key=lambda p: p[1], reverse=True)
        chosen_h = next(iter(h_values))[0]
        return chosen_h
